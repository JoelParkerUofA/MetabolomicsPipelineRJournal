...
## Metabolite analysis
At the metabolite level, we look at the pairwise comparisons for each of the experimental
conditions. Since we are making multiple comparisons for each metabolite, it is important
that we preserve the family wise error rate. To do this, we first test to see if the model containing 
the experimental groups explained a significant amount of variance in comparison to the intercept 
only model. 

## Box plots and heatmaps
After the sub pathway analysis or the metabolite pairwise comparisons, we may have
identified a specific sub pathway or metabolite that we would like to look at. The 
MetabolomicsPipline package provides wrapers around ggplot to easily plot line plots and 
box plots for metabolites within sub pathways. Analyst familular with ggplot can customize 
these plots for their specific needs. 
...
# Case Study
Here we will demonstrate how to utilize the tools within the MetabolomicsPipeiline
package via a case study consisting of metabololite measurements from 1102 metabolites
in 86 (44 female, 46 male) samples from mice. In this experiment there are three
different treatment groups (treat1, treat1, and control) and three different treatment
times (presymp, onset and treatment). 

We start by loading the data and showing the number of samples in each of the 
experimental groups. 
...
To explore sources of variation we can plot the first two pricipal components where
each samples is colored by GROUP_NAME, TIME1 and Gender. 

```{r PCA_Plots}
# Colored by GROUP_NAME
metabolite_pca(dat, Assay = "normalized", meta_var = "GROUP_NAME")


# Colored by TIME1
metabolite_pca(dat, Assay = "normalized", meta_var = "TIME1")
...
```
The plots above show that much of the variability in the metabolomic profiles is 
contributed to gender. Since gender is not of primary interest for this experiment 
then we are going to want stratify the down stream analysis. 

Now we look at how the top 50 metabololites are differing across GROUP_NAMEs and
TIME1 variable. As we demonstrated above, these heatmaps need to be stratified by
Gender. 
...
Next, we are going to test the interaction, parallel and single models at the sub
pathway level. As we identified above, we want to stratify this analysis by Gender. 
The subpathway_analysis function allows us to specify a variable to automatically 
stratify the analysis by. We also present the results of the subpathway analsis 
...
More sub pathways had at least one significant in males than females. Three super pathways
had at least one significant model in more than half of the sub pathways in both males
and females (peptide, amino acids, xenobiotics). Drug and topical agents is a subpthway
within the Xenobiotics super pathway and we can see for males the metabololites within 
this subpathway were only effected by time. Where in females were effected by time and treatment. 

Based on the results of the sub pathway analysis, the drug and topical sub pathway may be of
interest. We can visualize the data for each metabolite within the sub pathway using the 
subpathway_boxplots function or the subpathway_lineplots function.
